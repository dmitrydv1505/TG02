...
async def react_photo(message: Message):
    ...
    file_path = os.path.join('img', f'{message.photo[-1].file_id}.jpg')
    await bot.download(message.photo[-1], destination=file_path)

@dp.message(Command('photo', prefix='&'))
async def send_photo(message: Message):
    photos = [
        'https://content.onliner.by/news/original_size/53ddc2f05f38cfdaac5d30d288bd6edd.png',
...
# Функция для обработки неизвестных текстовых сообщений
@dp.message()
async def echo_unknown_command(message: Message):
